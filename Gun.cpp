...
namespace Tmpl8
{
	...
	void Firearm::evolve(int guntype)
	{
		...
		case 14: // sniper/shotgun, increased range, increased damage, decreased firerate/ammo
			if (weapon == 2) damage += 4, weapon = SniperShotgun; //if the old gun is a sniper, boost damage (due to evolving with shotgun). decreased firerate
			if (weapon == 3) range += 60, weapon = ShotgunSniper; //if the old gun is a shotgun, boost range (due to evolving with sniper). decreased ammo
			magazine = ammo, reloading = reloadrate, cooldown = firerate;
			barrelxleft = 384, barrelxright = 415, bheight1 = 256, bheight2 = -1;
			break;

		case 15: // sniper/minigun, either increased range and firerate or greatly increased range, decreased reloadrate
			if (weapon == 2) firerate -= 20, range += 60, reloadrate += 100, weapon = SniperMinigun; //if the old gun is a sniper, increase range, boost firerate (due to evolving with minigun). decreased reloadrate
			if (weapon == 4) range += 60, weapon = MinigunSniper; //if the old gun is a minigun, boost range (due to evolving with sniper). decreased reloadrate
			magazine = ammo, reloading = reloadrate, cooldown = firerate;
		...
		case 16: // sniper/twin guns, increased range, increased ammo, decreased firerate/speed
			if (weapon == 2) ammo += 4, weapon = SniperTwinguns; //if the old gun is a sniper, boost ammo (due to evolving with twin guns). decreased firerate
			if (weapon == 5) range += 60, weapon = TwingunSniper; //if the old gun is twin guns, boost range (due to evolving with sniper). decreased speed
			magazine = ammo, reloading = reloadrate, cooldown = firerate;
		...
		case 17: // shotgun/minigun, increased damage, increased firerate, decreased ammo/reloadrate
			if (weapon == 3) firerate -= 10, weapon = ShotgunMinigun; //if the old gun is a shotgun, boost firerate (due to evolving with minigun). decreased ammo
			if (weapon == 4) damage += 4, weapon = MinigunShotgun; //if the old gun is a minigun, boost damage (due to evolving with shotgun). decreased reloadrate
			magazine = ammo, reloading = reloadrate, cooldown = firerate;
		...
		case 18: // shotgun/twin guns, either increased damage and ammo or greatly increased damage, decreased speed
			if (weapon == 3) ammo += 2, damage += 4, speed -= 1, weapon = ShotgunTwinguns; //if the old gun is a shotgun, boost ammo (due to evolving with twin guns). decreased speed
			if (weapon == 5) damage += 4, weapon = TwingunShotgun; //if the old gun is twin guns, boost damage (due to evolving with shotgun). decreased speed
			magazine = ammo, reloading = reloadrate, cooldown = firerate;
		...
		case 19: // minigun/twin guns, increased firerate, increased ammo, decreased speed/reloadrate
			if (weapon == 4) ammo += 4, weapon = MinigunTwinguns; //if the old gun is a minigun, boost ammo (due to evolving with twin guns). decreased reloadrate
			if (weapon == 5) firerate -= 10, weapon = TwingunMinigun; //if the old gun is twin guns, boost firerate (due to evolving with minigun). decreased speed
			magazine = ammo, reloading = reloadrate, cooldown = firerate;
	...
	void Firearm::Shoot(Surface* screen, float charx, float chary, int i, float TimeMultiplier)
	{
		...
		for (int w = 0; w < 8; w++)
		{
			if (weapon == Shotguns[w]) { shotgun = 1; } //any shotgun variant shoot a double bullet
